		...
		<div class="warpperO">
			...
			<div class="contentWrapO">
				...
				<div class="pblicTermWrapO publicButtomLIne">
					...
					<div class="publicTermBoxO">
						<!--左-->
						<p class="publicTermLeftO">手机号码<span class="importantColor">*</span></p>
						<!--右-->
						<div class="publicTermRightO">
							<!--input-->
							<input type="phone" class="publicTROInput" v-model="userData.phone" placeholder="请输入手机号">
						</div>
					</div>
				</div>
                 <div class="pblicTermWrapP publicButtomLIne">
					...
					<div class="publicTermBoxP">
						...
						<div class="publicTermRightP shortWidthBox">
							<!--input-->
							<input type="text" class="publicTRPInput shortInputWidth" v-model="userData.phoneCode" placeholder="请输入验证码">
						</div>																		
						<!--'获取验证码'按钮-->
						<button v-bind:class="{obtainCodeBut:clickState, obtainCodeNoBut:!clickState}" @click="obtainCode" :disabled="!clickState" type="button">{{codeText}}</button>
				...
				<div class="pblicTermWrapO publicButtomLIne">
					...
					<div class="publicTermBoxO">
						...
						<div class="publicTermRightO">
							<!--input-->
							<input type="password" class="publicTROInput" name="pwd" autocomplete v-model="userData.passOne" placeholder="请输入登录密码">
						</div>
					</div>
				</div>
				<!--公共的class=>项Wrap，class="publicButtomLIne"=>(伪类)'底边框线'-->
				<div class="pblicTermWrapO">
					...
					<div class="publicTermBoxO">
						...
						<div class="publicTermRightO">
							<!--input-->
							<input type="password" class="publicTROInput" name="pwd2" autocomplete v-model="userData.passTwo" placeholder="请重复输入密码" >
						</div>
					</div>
				</div>
			...
			<!-- <div class="registerButBox"> -->
				<a href="javascript:;" class="registerButton" @click="registerBut">注册</a>
			<!-- </div> -->
</form>
			</div>
	...
	export default {
		...
		methods: {
			...
			obtainCode(){
				let that = this;
				var temp='sms_reg';
				var auth = md5( that.userData.phone + md5(temp+'android+app') );
				var phone = /^[1]([3-9])[0-9]{9}$/;
				if(that.userData['phone'] == ''){
					Toast('手机号不能为空！');
					return false;
				}else if(!phone.test(that.userData['phone'])){
					Toast('手机号码输入不规范,请输入正确的手机格式');
					return false;
				}else{ 
					if (that.clickState) {
					var url = "/api/PhoneAuth/verifycode?mobile="+that.userData.phone+'&temp='+temp+'&auth='+auth;
